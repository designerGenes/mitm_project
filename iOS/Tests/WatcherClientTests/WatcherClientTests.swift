...
/// Tests for the WatcherClient HTTP interactions using MockURLProtocol.
final class WatcherClientTests: XCTestCase {
    var client: WatcherClient!
    var session: URLSession!

    override func setUp() {
        super.setUp()
        session = MockURLProtocol.session()
        client = WatcherClient(port: 9090, session: session)
    }
    ...
    func testHTTPError() throws {
        ...
        XCTAssertThrowsError(try client.health()) { error in
            guard case WatcherError.unexpectedStatus(let code, _) = error else {
    ...
    func testConnectionError() throws {
        ...
        XCTAssertThrowsError(try client.health()) { error in
            guard case WatcherError.connectionFailed = error else {
    ...
    func testDecodingError() throws {
        ...
        XCTAssertThrowsError(try client.status()) { error in
            guard case WatcherError.decodingFailed = error else {
    ...
    // MARK: - Watcher Static Facade

    func testWatcherConfigureAndQuery() throws {
        Watcher.configure(port: 9090, session: session)
