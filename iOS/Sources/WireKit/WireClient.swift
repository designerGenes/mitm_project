...
/// Instance-based client for talking to the Wire daemon over HTTP.
///
...
public final class WireClient: @unchecked Sendable {
        ...
        let task = session.dataTask(with: request) { [decoder] data, response, error in
            ...
            if let error = error {
                ...
                if nsError.domain == NSURLErrorDomain &&
                    ...
                    result = .failure(WireError.connectionFailed(underlying: error))
                } else {
                    result = .failure(WireError.connectionFailed(underlying: error))
                }
                return
            ...
            guard let httpResponse = response as? HTTPURLResponse else {
                result = .failure(WireError.unexpectedStatus(-1, body: nil))
                return
            }

            guard let data = data else {
                result = .failure(WireError.unexpectedStatus(httpResponse.statusCode, body: nil))
                return
            }

            guard (200...299).contains(httpResponse.statusCode) else {
                let body = String(data: data, encoding: .utf8)
                result = .failure(WireError.unexpectedStatus(httpResponse.statusCode, body: body))
                return
            }
            ...
            do {
                ...
                result = .failure(WireError.decodingFailed(underlying: error))
            }
        }
        task.resume()
