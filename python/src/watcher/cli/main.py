"""CLI for the Watcher daemon: start|stop|reset|status."""
...
def _daemon_port() -> int:
    ...
    try:
        ...
        return int(plist.get("EnvironmentVariables", {}).get("WATCHER_API_PORT", "9090"))
    except Exception:
        return 9090
...
def start(port: int, proxy_port: int, output: str | None, verbose: bool, unsafe: bool, foreground: bool):
    ...
    if launchd.is_loaded():
        click.echo(f"Watcher is already running. Use 'watcher stop' first.")
        sys.exit(1)
    ...
    click.echo(f"Starting Watcher (API :{port}, proxy :{proxy_port})...")
    time.sleep(1)
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            click.echo("Watcher started but health check returned unexpected status.", err=True)
    except httpx.ConnectError:
        click.echo("Watcher loaded but not yet responding. Check logs at:")
        click.echo(f"  {launchd.LOG_DIR / 'watcher.stderr.log'}")
...
def status(port: int | None, as_json: bool):
    ...
    click.echo(f"Watcher is running")
